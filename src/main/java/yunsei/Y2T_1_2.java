	...
	public static final void main(String... args) throws Exception {
		...
		for ( int radius: Arrays.asList(100, 200, 300, 400) ) {
			...
			if ( buffereds == null ) {
				...
				buffereds = marmot.createDataSet(MULTI_RINGS, geomCol, srid, plan);
			}
			else {
				marmot.execute(plan);
			}
		...
		marmot.createDataSet(TEMP_JOINED, geomCol, srid, plan);
		...
		buffereds = marmot.createDataSet(RESULT, geomCol, srid, plan);
